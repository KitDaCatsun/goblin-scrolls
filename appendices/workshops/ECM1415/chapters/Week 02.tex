...
\begin{enumerate}
    \item \(\forall u\ldotp M(u)\)
    \item \(\forall u\ldotp \space  f \rightarrow A(u)\)
    \item \(S(firewall, diagnostic) \rightarrow S(\textit{proxy server}, diagnostic)\)
    \item \((T(100) \land \neg T(500) \land \neg S(\textit{proxy server}, diagnostic)) \rightarrow \exists r\ldotp S(r, normal)\)
\end{enumerate}

\tocsection{Nested Quantifiers}
...
\tocsection{Contradiction}
\tocsection{Proof by Cases}
\tocsection{Mistakes in Proofs}
